        ...
        <div v-else>
          <div v-for="(item, index) in cart.items" :key="index" class="cart-card">
            <div class="row">
              <router-link
                class="col-3 p-0"
                :to="{ name: 'ProductDetails', params: { slug: item.productt.slug, productId: item.productt.id } }"
              >
                <img
                  class="img-thumbnail border-0"
                  :src="item.productt.image"
          ...
          <div class="text-center">
            ...
            <div class="row">
              <div class="col-md-6 mb-md-0 mb-3">
                <router-link
                  class="btn btn-black w-100 px-0"
                  :to="{ name: 'Cart' }"
                >{{ $t("message.cart") }}</router-link>
              </div>
              <div class="col-md-6">
                <router-link
                  class="btn btn-black w-100 px-0"
                  :to="{ name: 'Checkout' }"
                >{{ $t("message.checkout") }}</router-link>
              </div>
            </div>
          </div>
        </div>
